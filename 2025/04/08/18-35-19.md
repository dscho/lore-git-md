...
| **Date** | 2025-04-09T00:05:19+05:30 |
| **Message-ID** | [CAF=ncLaCKv4uQYQUZH7Z1yq+AeVz68PRjoe700DHT0kQmMTD9Q@mail.gmail.com](https://lore.kernel.org/git/CAF=ncLaCKv4uQYQUZH7Z1yq+AeVz68PRjoe700DHT0kQmMTD9Q@mail.gmail.com) |

---

**Thread**: [thread](./../07/12-29-17.thread.md#:~:text=2025/04/08/18-35-19)
...
that’s really reassuring. I’m looking forward to contributing and
...
>
> Yes  I will elaborate on those details in my proposal .I'm currently proposing git repo-info, The name repo-info is self-explanatory and clearly indicates that the command is focused on querying structured metadata and configuration details of a Git repository.I suppose the default output should be plain strings NUL terminated like other plumbing commands .
>
...
Thank you — that’s a great point. I absolutely plan to involve the
mailing list early in the process. During the Community Bonding
Period, I’ll draft an initial design proposal outlining the command
...
Yes, I’ve taken a look at json-writer.[c,h] and plan to use Git’s
...
data across various commands in Git (e.g., git ls-remote --format=json
and git for-each-ref --format=json use this approach).
> >
> > ```
...
functionality—such as repository initialization, option parsing, and
JSON output helpers—into a unified structure. This will ensure that
...
schema for each subcommand’s output early in the project. As I
implement new features, I’ll ensure they adhere to this schema. Errors
...
Git’s standard mechanisms (die(), error(), return codes) for handling
...
>
> I  meant I’ll implement a subcommand dispatcher similar to how other built-ins do (like builtin/remote.c, which has subcommand dispatching logic). This helps organize the code cleanly and allows users to call git repo-info path, git repo-info env, etc.
>
> > ```
...
You're right — unit and functional tests should be written during each
feature’s development (as part of the patch series). What I meant here
was that Week 9 will focus on cross-subcommand integration tests — to
...
>
> - Karthik
> Thank you for the feedback! I will make sure I clarify the things that I wrote vaguely in this proposal
>
