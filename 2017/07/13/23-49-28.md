...
| **Date** | 2017-07-13T23:49:28Z |
| **Message-ID** | [20170713234930.949612-12-sandals@crustytoothpaste.net](https://lore.kernel.org/git/20170713234930.949612-12-sandals@crustytoothpaste.net) |

---

**Thread**: [thread](./23-49-28.thread.md#:~:text=2017/07/13/23-49-28)
...
@@ -4076,7 +4076,7 @@ static int build_fake_ancestor(struct apply_state *state, struct patch *list)
 			else
...
index 89fcb5b337..8943626e6c 100644
--- a/builtin/grep.c
+++ b/builtin/grep.c
...
index 8ca1de9894..60fac9d4dc 100644
--- a/builtin/log.c
+++ b/builtin/log.c
...
index 71fe092644..da7ac495ba 100644
--- a/cache.h
+++ b/cache.h
...
index cbfd689939..d29c997e7c 100644
--- a/commit.c
+++ b/commit.c
...
index 9da9040bf0..2fb0895e4d 100644
--- a/remote.c
+++ b/remote.c
...
index 9ff120b305..34bc81adf4 100644
--- a/revision.c
+++ b/revision.c
...
index 16d48a4fb3..f5e85a398d 100644
--- a/sequencer.c
+++ b/sequencer.c
...
index e7f7b12ceb..208c9b1bcf 100644
--- a/sha1_name.c
+++ b/sha1_name.c
@@ -10,7 +10,7 @@
 #include "dir.h"
...
@@ -505,8 +505,8 @@ int find_unique_abbrev_r(char *hex, const unsigned char *sha1, int len)
 		return 40;
...
@@ -579,10 +579,10 @@ static inline int push_mark(const char *string, int len)
...
@@ -596,14 +596,14 @@ static int get_sha1_basic(const char *str, int len, unsigned char *sha1,
...
@@ -645,7 +645,7 @@ static int get_sha1_basic(const char *str, int len, unsigned char *sha1,
 		int detached;
...
+			detached = (buf.len == GIT_SHA1_HEXSZ && !get_oid_hex(buf.buf, oid));
 			...
 			if (detached)
 				return 0;
@@ -654,18 +654,18 @@ static int get_sha1_basic(const char *str, int len, unsigned char *sha1,
...
+	     !get_short_oid(str, len, &tmp_oid, GET_SHA1_QUIETLY)))
 	...
 	if (reflog_len) {
@@ -697,7 +697,7 @@ static int get_sha1_basic(const char *str, int len, unsigned char *sha1,
 				return -1;
 			}
 		}
...
@@ -730,10 +730,10 @@ static int get_sha1_basic(const char *str, int len, unsigned char *sha1,
 }
...
@@ -743,13 +743,13 @@ static int get_parent(const char *name, int len,
 	if (parse_commit(commit))
 		return -1;
 	if (!idx) {
...
+			oidcpy(result, &p->item->object.oid);
 		...
 		p = p->next;
@@ -758,13 +758,13 @@ static int get_parent(const char *name, int len,
 }
...
+	ret = get_oid_1(name, len, &oid, GET_SHA1_COMMITTISH);
 	if (ret)
 		return ret;
 	commit = lookup_commit_reference(&oid);
@@ -776,7 +776,7 @@ static int get_nth_ancestor(const char *name, int len,
 			return -1;
...
@@ -805,7 +805,7 @@ struct object *peel_to_type(const char *name, int namelen,
 	}
 }
...
@@ -856,7 +856,7 @@ static int peel_onion(const char *name, int len, unsigned char *sha1,
...
@@ -866,7 +866,7 @@ static int peel_onion(const char *name, int len, unsigned char *sha1,
...
@@ -879,7 +879,7 @@ static int peel_onion(const char *name, int len, unsigned char *sha1,
 	if (!o)
 		return -1;
...
+	oidcpy(oid, &o->oid);
 	if (sp[0] == '/') {
 		/* "$commit^{/foo}" */
 		char *prefix;
@@ -895,14 +895,14 @@ static int peel_onion(const char *name, int len, unsigned char *sha1,
...
@@ -916,14 +916,14 @@ static int get_describe_name(const char *name, int len, unsigned char *sha1)
 			if (ch == 'g' && cp[-1] == '-') {
 				cp++;
 				len -= cp - name;
...
+static int get_oid_1(const char *name, int len, struct object_id *oid, unsigned lookup_flags)
 {
 	int ret, has_suffix;
 	const char *cp;
@@ -950,25 +950,25 @@ static int get_sha1_1(const char *name, int len, unsigned char *sha1, unsigned l
 		if (!num && len1 == len - 1)
 			num = 1;
 		if (has_suffix == '^')
...
+	return get_short_oid(name, len, oid, lookup_flags);
 ...
 /*
@@ -1005,7 +1005,7 @@ static int handle_one_ref(const char *path, const struct object_id *oid,
 	return 0;
 }
...
@@ -1045,7 +1045,7 @@ static int get_sha1_oneline(const char *prefix, unsigned char *sha1,
...
+		st = get_oid_committish(sb.buf, &oid_tmp);
 	...
 	if (st)
@@ -1150,7 +1150,7 @@ int get_oid_mb(const char *name, struct object_id *oid)
 	if (!one)
 		return -1;
...
+	if (get_oid_committish(dots[3] ? (dots + 3) : "HEAD", &oid_tmp))
 	...
 	if (!two)
@@ -1339,21 +1339,13 @@ int strbuf_check_branch_ref(struct strbuf *sb, const char *name)
 }
...
@@ -1367,49 +1359,49 @@ int get_oid(const char *name, struct object_id *oid)
...
+	struct object_id oid;
 	...
 	if (!prefix)
@@ -1421,8 +1413,8 @@ static void diagnose_invalid_sha1_path(const char *prefix,
...
+				    oid.hash, &mode)) {
 			die("Path '%s' exists, but not '%s'.\n"
 			    ...
 			    fullname,
@@ -1505,11 +1497,11 @@ static char *resolve_relative_path(const char *rel)
 			   rel);
 }
...
@@ -1522,7 +1514,7 @@ static int get_sha1_with_context_1(const char *name,
...
+	ret = get_oid_1(name, namelen, oid, flags);
 	if (!ret)
 		return ret;
 	/*
@@ -1542,7 +1534,7 @@ static int get_sha1_with_context_1(const char *name,
...
+			return get_oid_oneline(name + 2, oid, list);
 		...
 		if (namelen < 3 ||
 		    name[2] != ':' ||
@@ -1574,7 +1566,7 @@ static int get_sha1_with_context_1(const char *name,
...
+				oidcpy(oid, &ce->oid);
 				...
 				free(new_path);
 				return 0;
@@ -1595,14 +1587,14 @@ static int get_sha1_with_context_1(const char *name,
 			break;
 	}
 	if (*cp == ':') {
...
@@ -1610,20 +1602,20 @@ static int get_sha1_with_context_1(const char *name,
 			if (new_filename)
...
@@ -1647,13 +1639,13 @@ static int get_sha1_with_context_1(const char *name,
