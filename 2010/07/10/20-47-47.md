...
| **Date** | 2010-07-10T20:47:47Z |
| **Message-ID** | [1278794867-32438-1-git-send-email-avarab@gmail.com](https://lore.kernel.org/git/1278794867-32438-1-git-send-email-avarab@gmail.com) |

---

**Thread**: [thread](./20-47-47.thread.md#:~:text=2010/07/10/20-47-47)
...
This is version 6 of the gettext patch for Git. It replaces the
version already in pu.

I've upgraded the tests to work with my TAP series, which got merged
into master & next since I submitted v5.

This is the diff --stat since v5:

```
    t/lib-gettext.sh             |   42 +++++++++++++++++++++++
    t/t0200-gettext.sh           |   77 ++++++++++++++---------------------------
    t/t0201-gettext-fallbacks.sh |   11 +++---
    t/t0202-gettext-perl.sh      |   14 +++++---
    t/t0202/test.pl              |   25 ++++++++------
    5 files changed, 96 insertions(+), 73 deletions(-)

```

And the changes are, in more detail:

  * Added a t/lib-gettext.sh. It's used by the t/t02* tests,
    previously they duplicated some setup code (e.g. setting
    GIT_TEXTDOMAINDIR), now they don't.

  * Eliminated skip_all=<reason> in favor of using test_expect_success
    <prereq>. The same number of tests are now run on all platforms
    regardless of whether they support gettext or not. We just skip
    some of the tests now.

  * Copy the UTF-8 locale detection from
    t9129-git-svn-i18n-commitencoding.sh. Setting is_IS.UTF-8 was
    unportable, Debian now uses is_IS.utf8. Support both.

  * Use the new locale detection to always run certain tests if
    there's a UTF-8 is_IS locale on the system. The
    TEST_GIT_GETTEXT_EXHAUSTIVE=1 variable doesn't exist
    anymore. Instead we autodetect by shelling out to `locale -a`.

  * Remove the gettext("") test under the Klingon locale. I've found
    that using imaginary locales is unportable. This particular test
    failed on FreeBSD.

  * Added "Copyright (c) 2010 Ævar Arnfjörð Bjarmason" to the new *.sh
    test files, as recommended by t/README.

I've tested this on two Debian machines, one with a is_IS locale and
one not. I've also tested on FreeBSD and Solaris.

If you're interested in testing this series you can run this:

    curl http://github.com/avar/test-git-gettext/raw/master/test-git-gettext.sh | sh

Please provide the contents of /tmp/git-gettext.txt if $(grep 'not ok'
/tmp/git-gettext.txt) yields anything.
...
```
 ...
 t/test-lib.sh                |    2 +
 upload-pack.c                |    3 +
 32 files changed, 771 insertions(+), 4 deletions(-)
...
+#
 ...
 #
@@ -281,6 +290,7 @@ infodir = share/info
...
@@ -369,6 +381,7 @@ SCRIPT_SH += git-web--browse.sh
...
@@ -762,11 +787,13 @@ ifeq ($(uname_S),Linux)
...
+	NEEDS_LIBINTL =
 ...
 ifeq ($(uname_S),IRIX)
 	NO_SETENV = YesPlease
@@ -1456,6 +1484,14 @@ ifdef GIT_TEST_CMP_USE_COPIED_CONTEXT
...
@@ -1513,7 +1550,9 @@ template_dir_SQ = $(subst ','\'',$(template_dir))
...
@@ -1609,6 +1648,7 @@ sed -e '1s|#!.*/sh|#!$(SHELL_PATH_SQ)|' \
...
@@ -2204,6 +2265,10 @@ ifndef NO_TCLTK
...
+
 ...
 #
@@ -777,6 +783,12 @@ AC_CHECK_HEADER([paths.h],
 [HAVE_PATHS_H=])
