...
| **Date** | 2010-07-08T12:14:18-05:00 |
| **Message-ID** | [20100708171418.GA18229@burratino](https://lore.kernel.org/git/20100708171418.GA18229@burratino) |

---

**Thread**: [thread](./../06/22-55-22.thread.md#:~:text=2010/07/08/17-14-18)
...
‘dash’ might be good for testing.
...
@@ -144,10 +144,10 @@ rev=$(git describe ${merged} 2>/dev/null)
...
+author=$(printf "%s\n" "$rawcommit" | sed -n -e '/^author .*<\([^@]*\).*$/s--\1-p')
+logmessage=$(printf "%s\n" "$rawcommit" | sed -e '1,/^$/d' | head -n 1)
+logmessage=$(printf "%s\n" "$logmessage" | sed 's/\&/&amp\;/g; s/</&lt\;/g; s/>/&gt\;/g')
+ts=$(printf "%s\n" "$rawcommit" | sed -n -e '/^author .*> \([0-9]\+\).*$/s--\1-p')
...
+	printf 'git bisect start%s\n' "$orig_args" >>"$GIT_DIR/BISECT_LOG" || exit
 	#
...
+	printf '%s\n' "# $state: $(git show-branch $rev)" >>"$GIT_DIR/BISECT_LOG"
