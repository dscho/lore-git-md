# Re: make test fails with git 1.7.0 on my debian

| Header | Value |
|--------|-------|
| **From** | Sylvain Rabot <sylvain@abstraction.fr> |
| **To** | git@vger.kernel.org |
| **Date** | 2010-02-17T11:40:57+01:00 |
| **Message-ID** | [4B7BC7B9.3040706@abstraction.fr](https://lore.kernel.org/git/4B7BC7B9.3040706@abstraction.fr) |

---

**Thread**: [thread](./../16/16-18-07.thread.md#:~:text=2010/02/17/10-40-57)

Le 16/02/2010 17:23, Robin H. Johnson a écrit :
> On Tue, Feb 16, 2010 at 05:18:07PM +0100, Sylvain Rabot wrote:
> > root@numerobis:~/src/git-1.7.0# uname -a
> > Linux numerobis 2.6.26-2-686 #1 SMP Wed Nov 4 20:45:37 UTC 2009 i686 GNU/Linux
> > root@numerobis:~/src/git-1.7.0# make test
>
> Don't run the tests as root.
>


My mistake.

Thank you.


