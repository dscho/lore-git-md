...
| **Date** | 2016-04-27T16:04:32-04:00 |
| **Message-ID** | [1461787481-877-11-git-send-email-dturner@twopensource.com](https://lore.kernel.org/git/1461787481-877-11-git-send-email-dturner@twopensource.com) |

---

**Thread**: [thread](./20-04-22.thread.md#:~:text=2016/04/27/20-04-32)
...
--- a/read-cache.c
+++ b/read-cache.c
@@ -1234,7 +1234,7 @@ int refresh_index(struct index_state *istate, unsigned int flags,
 		if (!new) {
 			const char *fmt;
...
+			if (really || cache_errno == EINVAL) {
 				 ...
 				 */
@@ -1374,11 +1374,76 @@ static int verify_hdr(const struct cache_header *hdr, unsigned long size)
 	return 0;
 }
...
@@ -1408,10 +1473,24 @@ static int read_watchman_ext(struct index_state *istate, const void *data,
...
@@ -1644,28 +1723,90 @@ static void post_read_index_from(struct index_state *istate)
...
+
 ...
 {
@@ -1853,7 +2062,7 @@ int read_index_from(struct index_state *istate, const char *path)
...
+		goto done;
 	...
 	if (split_index->base)
@@ -1874,6 +2083,10 @@ int read_index_from(struct index_state *istate, const char *path)
