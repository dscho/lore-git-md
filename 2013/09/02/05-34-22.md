...
| **Date** | 2013-09-02T01:34:22-04:00 |
| **Message-ID** | [1378100066-31889-4-git-send-email-rhansen@bbn.com](https://lore.kernel.org/git/1378100066-31889-4-git-send-email-rhansen@bbn.com) |

---

**Thread**: [thread](./05-34-19.thread.md#:~:text=2013/09/02/05-34-22)
...
@@ -134,7 +134,7 @@ The `mode <mode>,<mode>..<mode>` line appears only if at least one of
...
+	      [--remote=<repo> [--exec=<git-upload-archive>]] <treeish>
 	      [<path>...]
...
+'git checkout' [-f|--ours|--theirs|-m|--conflict=<style>] [<treeish>] [--] <paths>...
+'git checkout' [-p|--patch] [<treeish>] [--] [<paths>...]
 
 DESCRIPTION
 -----------
@@ -76,14 +76,14 @@ the option, giving a branch name to the command would check out the branch,
...
@@ -245,7 +245,7 @@ leave out at most one of `A` and `B`, in which case it defaults to `HEAD`.
...
+'git diff-index' [-m] [--cached] [<common diff options>] <treeish> [<path>...]
...
+If there is only one <treeish> given, the commit is compared with its parents
 (see --stdin below).
...
+		[--error-unmatch] [--with-tree=<treeish>]
...
+modifying index and working tree to match.  The <treeish>/<commit> defaults
...
@@ -9,7 +9,7 @@ git-tar-tree - Create a tar archive of the files in the named tree object
 SYNOPSIS
 --------
 [verse]
-'git tar-tree' [--remote=<repo>] <tree-ish> [ <base> ]
+'git tar-tree' [--remote=<repo>] <treeish> [ <base> ]
...
@@ -212,7 +212,7 @@ ways--by the SHA-1 name for that tree, by the name of a commit that
...
@@ -147,7 +147,7 @@ it does not have to be dereferenced even once to get to an object.
...
+	N_("git archive --remote <repo> [--exec <cmd>] [options] <treeish> [<path>...]"),
 	N_("git archive --remote <repo> [--exec <cmd>] --list"),
 	NULL
 };
@@ -373,7 +373,7 @@ static int parse_archive_args(int argc, const char **argv,
 	if (!format)
 		format = "tar";
...
@@ -291,11 +291,11 @@ static int checkout_paths(const struct checkout_opts *opts,
 			 */
 			continue;
 		/*
...
--- a/builtin/diff.c
+++ b/builtin/diff.c
@@ -261,7 +261,7 @@ int cmd_diff(int argc, const char **argv, const char *prefix)
 	int result = 0;
...
@@ -319,10 +319,10 @@ void overlay_tree_on_cache(const char *tree_name, const char *prefix)
 	int i;
...
@@ -506,8 +506,8 @@ int cmd_ls_files(int argc, const char **argv, const char *cmd_prefix)
...
+	N_("git read-tree [[-m [--trivial] [--aggressive] | --reset | --prefix=<prefix>] [-u [--exclude-per-directory=<gitignore>] | -i]] [--no-sparse-checkout] [--index-output=<file>] (--empty | <treeish1> [<treeish2> [<treeish3>]])"),
 	NULL
 };
...
@@ -21,10 +21,10 @@ int cmd_tar_tree(int argc, const char **argv, const char *prefix)
 	 *
 	 * $0 --remote=<repo> arg... ==>
...
+	 * $0 treeish basedir ==>
+	 * 	git archive --format-tar --prefix=basedir treeish
 	 */
 	int i;
 	const char **nargv = xcalloc(sizeof(*nargv), argc + 3);
@@ -57,7 +57,7 @@ int cmd_tar_tree(int argc, const char **argv, const char *prefix)
...
+# or the named treeish (when rev is given).
 ...
 if test "$#" -ge 1
 then
@@ -126,9 +126,9 @@ Did you intend to checkout '$@' which can not be resolved as commit?"
 	fi
 	if test '' != "$new"
 	then
...
@@ -147,7 +147,7 @@ Did you intend to checkout '$@' which can not be resolved as commit?"
 	exit $?
 else
...
@@ -7969,7 +7969,7 @@ msgstr "Die Option --delete kann nur mit Referenzen verwendet werden."
 msgid ""
 "git read-tree [[-m [--trivial] [--aggressive] | --reset | --prefix=<prefix>] "
 "[-u [--exclude-per-directory=<gitignore>] | -i]] [--no-sparse-checkout] [--"
-"index-output=<file>] (--empty | <tree-ish1> [<tree-ish2> [<tree-ish3>]])"
+"index-output=<file>] (--empty | <treeish1> [<treeish2> [<treeish3>]])"
 msgstr ""
 "git read-tree [[-m [--trivial] [--aggressive] | --reset | --prefix=<Prefix>] "
 "[-u [--exclude-per-directory=<gitignore>] | -i]] [--no-sparse-checkout] [--"
...
+"git archive --remote <repo> [--exec <cmd>] [options] <treeish> [<path>...]"
 ...
 #: archive.c:13
@@ -5959,11 +5959,11 @@ msgid "if any <file> is not in the index, treat this as an error"
 msgstr ""
...
+"git archive --remote <repo> [--exec <cmd>] [options] <treeish> [<path>...]"
 ...
 #: archive.c:13
@@ -6084,11 +6084,11 @@ msgid "if any <file> is not in the index, treat this as an error"
 msgstr ""
...
+"git archive --remote <repo> [--exec <cmd>] [options] <treeish> [<path>...]"
 ...
 "[<sökväg>...]"
@@ -6334,11 +6334,11 @@ msgid "if any <file> is not in the index, treat this as an error"
...
+msgid "git ls-tree [<options>] <treeish> [<path>...]"
 ...
 #: builtin/ls-tree.c:125
@@ -7770,7 +7770,7 @@ msgstr "--delete kan inte användas utan referenser"
 msgid ""
 "git read-tree [[-m [--trivial] [--aggressive] | --reset | --prefix=<prefix>] "
 "[-u [--exclude-per-directory=<gitignore>] | -i]] [--no-sparse-checkout] [--"
-"index-output=<file>] (--empty | <tree-ish1> [<tree-ish2> [<tree-ish3>]])"
...
+msgid "git archive [options] <treeish> [<path>...]"
+msgstr "git archive [các-tùy-chọn] <treeish> [<đường-dẫn>...]"
...
+"git archive --remote <kho> [--exec <lệnh>] [các-tùy-chọn] <treeish> [<đường-"
 "dẫn>...]"
 
 #: archive.c:13
@@ -6450,21 +6450,21 @@ msgid "if any <file> is not in the index, treat this as an error"
...
+"giả định rằng các đường dẫn đã bị gỡ bỏ kể từ <treeish> nay vẫn hiện diện"
...
+msgid "git ls-tree [<options>] <treeish> [<path>...]"
+msgstr "git ls-tree [<các-tùy-chọn>] <treeish> [<đường-dẫn>...]"
 
 #: builtin/ls-tree.c:125
 msgid "only show trees"
@@ -7912,12 +7912,11 @@ msgstr "--delete không hợp lý nếu không có bất kỳ tham chiếu (refs
 msgid ""
 "git read-tree [[-m [--trivial] [--aggressive] | --reset | --prefix=<prefix>] "
 "[-u [--exclude-per-directory=<gitignore>] | -i]] [--no-sparse-checkout] [--"
-"index-output=<file>] (--empty | <tree-ish1> [<tree-ish2> [<tree-ish3>]])"
...
-"ish3>]])"
+"[--index-output=<tập-tin>] (--empty | <treeish1> [<treeish2> [<treeish3>]])"
...
+msgid "git reset [-q] <treeish> [--] <paths>..."
+msgstr "git reset [-q] <treeish> [--] <đường-dẫn>..."
 
 #: builtin/reset.c:27
-msgid "git reset --patch [<tree-ish>] [--] [<paths>...]"
-msgstr "git reset --patch [<tree-ish>] [--] [<các-đường-dẫn>...]"
+msgid "git reset --patch [<treeish>] [--] [<paths>...]"
+msgstr "git reset --patch [<treeish>] [--] [<các-đường-dẫn>...]"
...
@@ -6338,11 +6338,11 @@ msgid "if any <file> is not in the index, treat this as an error"
...
+msgid "git ls-tree [<options>] <treeish> [<path>...]"
 ...
 #: builtin/ls-tree.c:125
@@ -7740,7 +7740,7 @@ msgstr "--delete 未接任何引用没有意义"
 msgid ""
 "git read-tree [[-m [--trivial] [--aggressive] | --reset | --prefix=<prefix>] "
 "[-u [--exclude-per-directory=<gitignore>] | -i]] [--no-sparse-checkout] [--"
-"index-output=<file>] (--empty | <tree-ish1> [<tree-ish2> [<tree-ish3>]])"
...
@@ -45,12 +45,12 @@ test_expect_success 'warn ambiguity when no candidate matches type hint' '
...
@@ -2931,7 +2931,7 @@ test_expect_success 'S: ls with garbage after sha1 must fail' '
 	ls ${sha1}x hello.c
 	EOF
 	cat err &&
