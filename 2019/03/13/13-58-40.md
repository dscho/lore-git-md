...
| **Date** | 2019-03-13T13:58:40Z |
| **Message-ID** | [SN6PR03MB364786545AA1170012EEB5F9BA4A0@SN6PR03MB3647.namprd03.prod.outlook.com](https://lore.kernel.org/git/SN6PR03MB364786545AA1170012EEB5F9BA4A0@SN6PR03MB3647.namprd03.prod.outlook.com) |

---

**Thread**: [thread](./../../02/22/03-36-45.thread.md#:~:text=2019/03/13/13-58-40)
...
IIRC we fixed some potentially unterminated string recently, and that might be the fix for your problem.
...
> To: Garcia, Pierre <pgarcia@trusimulation.com>
> Cc: git@vger.kernel.org
> Subject: Re: Bug: git for Windows spawning a lot of processes 
> recursively
>
> Hi Pierre,
>
...
> > ------
> > Issue:
> > When running from Git-bash (not even inside a repo), git commands 
> > from built-in are OK, but if I use the exe that is located at 
> > C:\Program Files\Git\bin\git.exe, the process takes long to execute 
> > and then exits with error "error launching git: The filename or 
> > extension is too
> > long.":
>
>
...
> > /mingw64/bin/git
> >
> > $ /c/Program\ Files/Git/bin/git.exe --version error launching git: 
...
> > --------------------------------------------------------------------
> > --
> > ------ I ran procmon.exe to monitor the activity of the process and 
> > I found out that git.exe was spawning itself 385 times in total in the trace I took, until all processes exit with error code 1, here is an extract of the procmon trace, showing thread and process activity for git.exe (I have more exhaustive trace but the file is quite big), I included the environment variables on the first call:
> >
...
> > --------------------------------------------------------------------
> > --
> > ------ For reference, here is what the built-in git --version is 
> > doing from git bash, only two processes are spawned, git --version first (the command line), then git rev-parse, and then the process exits:
> >
