# [PATCH 3/3] Documentation nitpicking

| Header | Value |
|--------|-------|
| **From** | Nikolai Weibull <mailing-lists.git@rawuncut.elitemail.org> |
| **To** | git <git@vger.kernel.org> |
| **Date** | 2005-11-15T00:20:01+01:00 |
| **Message-ID** | [1132010401671-git-send-email-mailing-lists.git@rawuncut.elitemail.org](https://lore.kernel.org/git/1132010401671-git-send-email-mailing-lists.git@rawuncut.elitemail.org) |

---

**Thread**: [thread](./23-20-01.thread.md#:~:text=2005/11/14/23-20-01-3)

This patch fixes some small problems with the documentation.

Signed-off-by: Nikolai Weibull <nikolai@bitwi.se>

---

```
 Documentation/git-commit-tree.txt  |    2 +-
 Documentation/git-update-index.txt |    2 +-
 2 files changed, 2 insertions(+), 2 deletions(-)

```

applies-to: 118fb90e721428900a0588074894a4b198044762
683a61f6648873de0a73c8c60a86064cf4f4911d
```diff
diff --git a/Documentation/git-commit-tree.txt b/Documentation/git-commit-tree.txt
index b64cd6a..5cf6bd3 100644
--- a/Documentation/git-commit-tree.txt
+++ b/Documentation/git-commit-tree.txt
@@ -8,7 +8,7 @@ git-commit-tree - Creates a new commit o
 
 SYNOPSIS
 --------
-'git-commit-tree' <tree> [-p <parent commit>]\   < changelog
+'git-commit-tree' <tree> [-p <parent commit>]\* < changelog
 
 DESCRIPTION
 -----------
diff --git a/Documentation/git-update-index.txt b/Documentation/git-update-index.txt
index 52874c8..fdcb8be 100644
--- a/Documentation/git-update-index.txt
+++ b/Documentation/git-update-index.txt
@@ -60,7 +60,7 @@ OPTIONS
 	Directly insert the specified info into the index.
 	
 --index-info::
-        Read index info from stdin.
+        Read index information from stdin.
 
 --chmod=(+|-)x::
         Set the execute permissions on the updated files.        
---
```

0.99.9g


